...
本文档为**微信小程序**接入 [turbo 引力引擎](https://gravity-engine.com/)的技术接入方案，具体 Demo 请参考[GitHub](https://github.com/GravityInfinite/Turbo-MiniProgram-Demo)。
...
- 从[GitHub](https://github.com/GravityInfinite/Turbo-MiniProgram-Demo/releases)上获取微信小程序SDK源码；
- 将`turbo.min.js`文件放入小程序项目中；
- 在app.js文件中通过import引入SDK
 ...
 */
turbo.setPara({
  autoTrack: { // autoTrack参数 可用于配置需要开启的全埋点类型：
    ...
    pageLeave: true // 默认为 true，false 则关闭 $MPPageLeave 事件采集
  },
 ...
 * @param {string} client_id      用户唯一标识，如微信小程序的openid
 */
...
#### 1.4 配置项目合法域名
将 https://turbo.api.plutus-cat.com 配置到微信后台request合法域名列表中。
...
#### 2.2 用户注册事件上报
当用户注册成功时，需要调用registerEvent方法记录用户注册事件
```javascript
turbo.registerEvent();
```
#### 2.3 用户登录事件上报
当用户登录成功时，需要调用loginEvent方法记录用户登录事件
```javascript
turbo.loginEvent();
```
#### 2.4 用户注销登录事件上报
当用户注销登录时，需要调用logoutEvent方法记录用户登出事件
```javascript
turbo.logoutEvent();
```
#### 2.5 买量埋点事件上报
