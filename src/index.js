...
var onLoadOption = null;
var show_time = null;

function monitorHooks(option) {
  var oldOnLoad = option["onLoad"];
  ...
  var oldShow = option["onShow"];
  option["onShow"] = function () {
    show_time = new Date().getTime();
    if (oldShow) {
      oldShow.apply(this, arguments);
    }
    if (turbo._para.autoTrack.pageShow) {
      turbo.track("$MPViewScreen", {
        $url_query: setQuery(onLoadOption || {}),
      });
    }
  };
  ...
  option["onHide"] = function () {
    ...
    if (turbo._para.autoTrack.pageLeave) {
      let event_duration = null;
      const current_time = new Date().getTime();
      if (
        show_time &&
        current_time - show_time > 0 &&
        (current_time - show_time) / 3600000 < 24
      ) {
        event_duration = current_time - show_time;
      }
      turbo.track("$MPPageLeave", {
        event_duration,
      });
    }
  };

  var oldShare = option["onShareAppMessage"];
  ...
  var oldOnShareTimeline = option["onShareTimeline"];
  option["onShareTimeline"] = function () {
    if (oldOnShareTimeline) {
      oldOnShareTimeline.apply(this, arguments);
    }
    if (turbo._para.autoTrack.appShare) {
      turbo.track("$MPShare", {
        $share_method: "朋友圈分享",
  ...
  var oldonAddToFavorites = option["onAddToFavorites"];
  option["onAddToFavorites"] = function () {
    if (oldonAddToFavorites) {
      oldonAddToFavorites.apply(this, arguments);
    }
